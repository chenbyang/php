...
if (!$_SESSION["id"]) { //如果Session沒有id
   ...
   #mysqli_connect() 建立資料庫的連結
   $conn=mysqli_connect("db4free.net", "immust", "immustimmust", "immust"); //建立資料庫的連結
   ...
   if (!mysqli_query($conn, $sql)) { //如果查詢指令錯誤
     echo "新增命令錯誤"; //印出新增的命令錯誤
   }
   else{ //如果成功
     echo "新增使用者成功，三秒鐘後回到網頁"; //印出新增使用者成功，三秒後跳回到網頁
